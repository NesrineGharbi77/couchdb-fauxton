...
export default class DocEditorScreen extends React.Component {
  ...
  getCodeEditor = () => {
    ...
    const code = preciseJSON.stringify(docContent, null, '  ');
  ...
  UNSAFE_componentWillUpdate(nextProps) {
    ...
    if (this.props.numFilesUploaded !== nextProps.numFilesUploaded ||
      ...
      if (editor && nextProps.doc) {
        this.getEditor().setValue(preciseJSON.stringify(nextProps.doc.attributes, null, '  '));
      }
      this.onSaveComplete();
    }
  }
  ...
  checkDocIsValid = () => {
    ...
    const json = preciseJSON.parse(this.getEditor().getValue());
